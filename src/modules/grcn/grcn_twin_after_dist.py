...
class GraphConvolutionResourceNetworkTwinAfterDist(nn.Module):
    ...
    def forward(self, state):
        ...
        if not self.other_agent_after_q:
            y = y.sum(dim=2)
            # x = torch.exp(x) * 1/torch.exp(y)
            if self.multiply_with_other:
                x = torch.exp(x) * 1 / (self.other_weight * torch.exp(y))
            else:
                x = x - (self.other_weight * y)
            ...
            current_agent_id_one_hot = F.one_hot(current_agent_id, self.n_agents) \
                .unsqueeze(-2) \
                .expand(*x.shape[:3], self.n_agents)
            distance_to_action_other_agents = state["distance_to_action_other_agents"]
            other_agent_ids = state["other_agent_ids"]
            other_agent_ids_one_hot = F.one_hot(other_agent_ids, self.n_agents).expand(*y.shape[:4], self.n_agents)

            if self.add_distance_to_action:
                input_q_net_current_agent = [x, distance_to_action.unsqueeze(-1), current_agent_id_one_hot]
                input_q_net_other_agent = [y, distance_to_action_other_agents.unsqueeze(-1), other_agent_ids_one_hot]
            else:
                input_q_net_current_agent = [x, current_agent_id_one_hot]
                input_q_net_other_agent = [y, other_agent_ids_one_hot]


            q_current = self.q_net(torch.cat(input_q_net_current_agent, dim=-1))
            q_other = self.q_net(torch.cat(input_q_net_other_agent, dim=-1))
            q_other = q_other.sum(dim=2)
            if self.multiply_with_other:
                q = torch.exp(q_current) * 1 / (self.other_weight * torch.exp(q_other))
            else:
                q = q_current - (self.other_weight * q_other)
